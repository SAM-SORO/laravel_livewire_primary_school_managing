...
<div class="p-2 bg-white shadow-sm">
    ...
    <form method="POST" wire:submit.prevent="store" id="inscriptionForm" autocomplete="off">
        ...
        <div class="block px-5 mb-5">
            ...
            <div class="relative">
                <select wire:model.lazy="idEleve" id="eleve" class="block appearance-none w-full bg-white border border-gray-300 hover:border-gray-400 px-4 py-2 pr-8 rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:ring focus:ring-blue-200" required value="{{old('eleve')}}">
                    <option value="" selected>Sélectionner le eleve</option>
                    @foreach ($eleves as $eleve)
                        <option value="{{ $eleve->id }}" class="py-2 px-4 transition duration-300 ease-in-out transform hover:bg-gray-100">{{ $eleve->nom }}</option>
                    @endforeach
                </select>
        ...
        <div class="block px-5 mb-5">
            ...
            <div class="relative">
                <select wire:model.lazy="idClasse" id="classe" class="block appearance-none w-full bg-white border border-gray-300 hover:border-gray-400 px-4 py-2 pr-8 rounded-md shadow-sm focus:outline-none focus:border-blue-500 focus:ring focus:ring-blue-200" required>
                    <option value="" selected>Sélectionner la classe d'affectation</option>
